...
from models import mar_order_z
from engine_mar_order_z import train_one_epoch, evaluate
import copy
...
def get_args_parser():
    parser = argparse.ArgumentParser('MAR Order Learning', add_help=False)
    parser.add_argument('--batch_size', default=16, type=int,
                        help='Batch size per GPU (effective batch size is batch_size * # gpus')
    parser.add_argument('--epochs', default=50, type=int, help="Epochs for order head training")

    # Model parameters
    parser.add_argument('--model', default='mar_large', type=str, metavar='MODEL',
                        help='Name of model to train')
    
    # Path to PRETRAINED MAR MODEL (Crucial)
    ...
    parser.add_argument('--order_mode', default='random', type=str,
                        help='Ordering mode (random vs. gradient)')
    ...
    parser.add_argument('--output_dir', default='./output_dir_learned_order',
                        help='path where to save')
    parser.add_argument('--log_dir', default='./output_dir_learned_order',
    ...
    parser.add_argument('--wandb_project', default='mar_order_z', type=str)
...
def main(args):
    ...
    model = mar_order_z.__dict__[args.model](
    ...
    # --- WANDB WATCH ---
    if global_rank == 0 and args.use_wandb and _WANDB_AVAILABLE and args.wandb_watch_grad:
         wandb.watch(model.order_head, log="all", log_freq=500)
    # -------------------
    ...
    # Pass only trainable params (the head) to optimizer
    ...
    if args.resume and os.path.exists(os.path.join(args.resume, "checkpoint-last.pth")):
        ...
        # NOTE: ema params here should strictly be only HEAD if we want to be pure,
        # but simpler to keep structure.
        # However, since body is frozen, EMA of body is just the body itself.
    ...
    # Init EMA params (just copy current state)
    ...
    # training
    print(f"Start training Order Head for {args.epochs} epochs")

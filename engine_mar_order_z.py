...
def train_one_epoch(model, vae,
    ...
    # We want the body to be in eval mode (Batch norms etc fixed), 
    # but the order head to be in train mode.
    model.eval() 
    if hasattr(model, 'module'):
        model.module.order_head.train()
    else:
        model.order_head.train()
        
    metric_logger = misc.MetricLogger(delimiter="  ")
    metric_logger.add_meter('lr', misc.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    header = 'Epoch: [{}] (Order Learning)'.format(epoch)
    print_freq = 20
    ...
    for data_iter_step, (samples, labels) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.no_grad():
            ...
            # normalize the std of latent to be 1
            x = posterior.sample().mul_(0.2325)

        # Forward pass specialized for training the Order Head
        with torch.cuda.amp.autocast():
            # This calls the new method we defined in MAR class
        ...
        # Optimizer update (only updates head because of how we set up the optimizer in main)
        ...
        if log_writer is not None:
            ...
            log_writer.add_scalar('order_head_loss', loss_value_reduce, epoch_1000x)
            log_writer.add_scalar('lr', lr, epoch_1000x)
        
        global_step = data_iter_step + len(data_loader) * epoch

        # --- WANDB LOGGING (SCALARS) ---
        # We check: 
        # 1. Are we the main process? (rank 0)
        # 2. Is wandb enabled?
        # 3. Is it time to log based on frequency?
        if misc.get_rank() == 0 and args.use_wandb and _WANDB_AVAILABLE:
            if global_step % args.wandb_log_freq == 0:
                wandb.log({
...
def evaluate(model_without_ddp, vae, ema_params, args, epoch, batch_size=16, log_writer=None, cfg=1.0,
    ...
    # NOTE: In this modified version, we might not use EMA for the Order Head 
    # if it's not strictly necessary, or we should handle the EMA copying carefully.
    # For now, we assume ema_params might just be the full model copy.
    
    if use_ema and ema_params is not None:
        model_state_dict = copy.deepcopy(model_without_ddp.state_dict())
        ema_state_dict = copy.deepcopy(model_without_ddp.state_dict())
        # This mapping assumes ema_params matches model.parameters() list order
    ...
    for i in range(num_steps):
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast():
                # sample_tokens now internally calls get_learned_orders
        ...
        # --- WANDB IMAGE LOGGING ---
        # Only log the FIRST batch (i==0) of the evaluation loop
        # Only log on Rank 0
        if i == 0 and misc.get_rank() == 0 and args.use_wandb and args.wandb_log_images and _WANDB_AVAILABLE:
            images_to_log = []
            # Log up to 8 images max to save space
            num_log = min(8, sampled_images.size(0))
            
            for idx in range(num_log):
                # Convert tensor to numpy for WandB
                # Tensor: [3, H, W] -> Numpy Transpose: [H, W, 3]
                img_np = np.clip(sampled_images[idx].numpy().transpose([1, 2, 0]), 0, 1)
                
                caption = f"Epoch_{epoch}_Cfg_{cfg}_Idx_{idx}"
                images_to_log.append(wandb.Image(img_np, caption=caption))
            
            postfix = "_ema" if use_ema else ""
            wandb.log({f"eval/generated_samples{postfix}": images_to_log}, step=epoch)
        # ---------------------------
